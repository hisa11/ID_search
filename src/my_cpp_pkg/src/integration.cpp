...
{
    return discoverMicrocontrollerIDs(timeout_ms, true); // デフォルトでクリアする
}

std::vector<std::string> IntegratedCommunicationSystem::discoverMicrocontrollerIDs(int timeout_ms, bool clear_existing)
{
    RCLCPP_INFO(this->get_logger(), "🔍 マイコン探索を開始します (timeout: %dms, clear_existing: %s)", 
    ...
    for(auto& dev : serial_comms_) {
        if (dev && dev->isOpen()) {
            RCLCPP_INFO(this->get_logger(), "  📡 Sending discovery command to %s", dev->getPort().c_str());
            ...
            if (dev->sendRaw(id_request_command)) {
                ...
                RCLCPP_WARN(this->get_logger(), "  ⚠️ Failed to send discovery command to %s", dev->getPort().c_str());
            }
        } else {
            RCLCPP_WARN(this->get_logger(), "  ❌ Device %s is not connected", dev ? dev->getPort().c_str() : "unknown");
        }
    }
    
    if (successful_sends == 0) {
        RCLCPP_ERROR(this->get_logger(), "❌ No devices available for discovery");
        discovery_promise_ = nullptr;
        return getDiscoveredMicrocontrollers();
    }
    
    RCLCPP_INFO(this->get_logger(), "  📤 Discovery commands sent to %d devices, waiting for responses...", successful_sends);
    ...
    if (wait_result == std::future_status::timeout) {
        RCLCPP_WARN(this->get_logger(), "⏰ Discovery timeout after %dms. Found %zu microcontrollers", timeout_ms, discovered.size());
    } else {
        RCLCPP_INFO(this->get_logger(), "✅ Discovery completed. Found %zu microcontrollers", discovered.size());
    }
    
    // 発見されたマイコンをログ出力
    for (const auto& mc : discovered) {
        RCLCPP_INFO(this->get_logger(), "  🔗 Discovered: %s", mc.c_str());
    }
    
    // 前回と結果が異なる場合は警告
    if (discovered.size() != previous_microcontrollers.size()) {
        RCLCPP_WARN(this->get_logger(), "📊 Microcontroller count changed: %zu -> %zu", 
...
{
    RCLCPP_INFO(this->get_logger(), "🔍 サービスリクエスト受信: source=%s, request_type=%ld", 
    ...
    if (request->request_type == 2) {
...

        {
            ...
            // デバッグ用：現在発見されているマイコンをログ出力
            RCLCPP_DEBUG(this->get_logger(), "📊 Current discovered microcontrollers:");
            for (const auto& mc : discovered_microcontrollers_) {
                RCLCPP_DEBUG(this->get_logger(), "  - %s", mc.c_str());
            }
        }

        RCLCPP_INFO(this->get_logger(), "🔍 (Type 2) Processing command for '%s', is_local=%s", 
                   target_mc.c_str(), is_local ? "true" : "false");

        if (is_local) {
            // ★★★ 配列をカンマ区切りの文字列に変換して直接シリアル送信 ★★★
            RCLCPP_INFO(this->get_logger(), "📥 (Type 2) Local command for '%s'", target_mc.c_str());
            ...
            if (!forward_to_node.empty()) {
                RCLCPP_INFO(this->get_logger(), "➡️ (Type 2) Forwarding command for '%s' to node '%s'", target_mc.c_str(), forward_to_node.c_str());
                ...
                RCLCPP_ERROR(this->get_logger(), "❌ (Type 2) Destination MC '%s' not found in network or only available locally.", target_mc.c_str());
    ...
    if (request->request_type == 3) { // メッセージバッファからの取得
        RCLCPP_INFO(this->get_logger(), "🔍 (Type 3) Getting buffered messages");
        ...
        RCLCPP_INFO(this->get_logger(), "📤 (Type 3) Returned %zu buffered messages", buffered_messages.size());
        return;
    }

    if (request->request_type == 102) { // マイコンからの応答処理
        if (data_handler_callback_) {
            data_handler_callback_(request);
        } else {
            RCLCPP_WARN(this->get_logger(), "データハンドラーが設定されていません");
        }
    } else { // その他のタイプは従来通りキューイング
        {
...
void IntegratedCommunicationSystem::handle_serial_data(const std::string& device_port, const std::string& raw_data) {
    RCLCPP_DEBUG(this->get_logger(), "📥 Received serial data from %s: '%s'", device_port.c_str(), raw_data.c_str());
    
    std::string data = raw_data;
    if (data.empty() || data.back() != '|') {
        RCLCPP_DEBUG(this->get_logger(), "⚠️ Invalid data format (no terminating '|'): '%s'", raw_data.c_str());
        return;
    }
    data.pop_back();
    ...
    RCLCPP_DEBUG(this->get_logger(), "📊 Parsed %zu parts from serial data", parts.size());
    for (size_t i = 0; i < parts.size(); ++i) {
        RCLCPP_DEBUG(this->get_logger(), "  Part[%zu]: '%s'", i, parts[i].c_str());
    }

    if (parts.size() >= 2 && parts[0] == "101") {
        RCLCPP_INFO(this->get_logger(), "🔍 Processing ID discovery response: ID=101");
        ...
        if (newly_discovered) {
            ...
            if (discovery_promise_) { 
                RCLCPP_INFO(this->get_logger(), "🎯 Discovery promise fulfilled for '%s'", mc_id.c_str());
    ...
    if (parts.size() >= 1 && parts[0] == "2") {
        RCLCPP_INFO(this->get_logger(), "📥 ID2メッセージをバッファリング: %s", raw_data.c_str());
        {
            std::lock_guard<std::mutex> lock(message_buffer_mutex_);
    ...
    if (data_handler_callback_) {
        ...
        // 元の生データをそのままPCに転送
        RCLCPP_INFO(this->get_logger(), "📤 マイコン '%s' からのデータをPCに転送: %s", source_mc_id.c_str(), raw_data.c_str());
        ...
        dummy_request->message = raw_data; // 元の文字列をそのまま保存
...
void IntegratedCommunicationSystem::monitoringLoop() {
    RCLCPP_INFO(this->get_logger(), "🔄 定期的なモニタリングループを開始しました");
    ...
    while (monitoring_enabled_ && !shutdown_flag_) {
        ...
        // シリアルデバイスの実際の接続状態もチェック
        bool serial_connection_issue = checkSerialConnectionHealth();
        
        if (network_changed || serial_changed || serial_connection_issue) {
            RCLCPP_INFO(this->get_logger(), "🔄 変更を検出: ネットワーク=%s, シリアル=%s, 接続問題=%s - 再探索を実行します", 
                       network_changed ? "変更あり" : "変更なし", 
                       serial_changed ? "変更あり" : "変更なし",
                       serial_connection_issue ? "検出" : "なし");
            
            if (serial_changed || serial_connection_issue) {
                rescanSerialDevices();
                // シリアル問題がある場合のみ、影響を受けるマイコンのマップを部分的にクリア
                RCLCPP_INFO(this->get_logger(), "🔄 シリアル問題によりマイコン探索を実行します");
                
                // 再探索を実行（マイコンマップはクリアしない、additive discovery）
                discoverMicrocontrollerIDs(2000, false); // クリアしない
                discoverNodesAndTheirMicrocontrollers(2000);
            } else {
                // 変更がなくても定期的に探索を実行（マイコンマップはクリアしない）
                RCLCPP_DEBUG(this->get_logger(), "🔄 定期探索を実行中...");
                discoverMicrocontrollerIDs(1000, false); // クリアしない
            }
        }
    }
    
    RCLCPP_INFO(this->get_logger(), "🔄 モニタリングループを終了しました");
}
...
bool IntegratedCommunicationSystem::detectNetworkChanges() {
    ...
    if (current_nodes.size() != last_known_nodes_.size()) {
        RCLCPP_INFO(this->get_logger(), "📊 ノード数の変化を検出: %zu -> %zu", 
    ...
    if (current_nodes != sorted_last_nodes) {
        RCLCPP_INFO(this->get_logger(), "📊 ノード構成の変化を検出");
        last_known_nodes_ = this->get_node_names(); // 元の順序で保存
        return true;
    }
...
bool IntegratedCommunicationSystem::detectSerialDeviceChanges() {
    ...
    if (current_ports.size() != last_known_serial_ports_.size()) {
        RCLCPP_INFO(this->get_logger(), "🔌 シリアルポート数の変化を検出: %zu -> %zu", 
    ...
    if (current_ports != sorted_last_ports) {
        RCLCPP_INFO(this->get_logger(), "🔌 シリアルポート構成の変化を検出");
...
void IntegratedCommunicationSystem::rescanSerialDevices() {
    RCLCPP_INFO(this->get_logger(), "🔌 シリアルデバイスの再スキャンを実行中...");
    
    // 既存のシリアルデバイスの再接続を試行
    for (auto& dev : serial_comms_) {
        if (dev) {
            RCLCPP_INFO(this->get_logger(), "  🔄 %s の再接続を試行中...", dev->getPort().c_str());
            
            if (!dev->isOpen() || !dev->checkConnection()) {
                RCLCPP_WARN(this->get_logger(), "  ⚠️ %s に問題が検出されました。再接続を試行します。", dev->getPort().c_str());
                
                if (dev->reconnect()) {
                    RCLCPP_INFO(this->get_logger(), "  ✅ %s の再接続に成功しました。", dev->getPort().c_str());
                    ...
                    RCLCPP_ERROR(this->get_logger(), "  ❌ %s の再接続に失敗しました。", dev->getPort().c_str());
                }
            } else {
                RCLCPP_INFO(this->get_logger(), "  ✅ %s は正常に動作しています。", dev->getPort().c_str());
            }
        }
    }
    ...
    for (const auto& port : ports_to_scan) {
        if (std::find(existing_ports.begin(), existing_ports.end(), port) == existing_ports.end()) {
            RCLCPP_INFO(this->get_logger(), "  🔍 新しいデバイスを検索中: %s", port.c_str());
            ...
            if (new_device->initialize()) {
                RCLCPP_INFO(this->get_logger(), "  ✅ 新しいデバイスを発見: %s", port.c_str());
    ...
    RCLCPP_INFO(this->get_logger(), "🔌 シリアルデバイスの再スキャン完了。現在 %zu 個のデバイスが接続されています。", serial_comms_.size());
}

bool IntegratedCommunicationSystem::checkSerialConnectionHealth() {
    bool has_issues = false;
    
    // 現在発見されているマイコンの数をチェック
    ...
    // シリアルデバイスは接続されているがマイコンが見つからない場合
    if (serial_comms_.size() > 0 && current_mc_count == 0) {
        RCLCPP_WARN(this->get_logger(), "⚠️ シリアルデバイスは接続されていますが、マイコンが見つかりません。再探索が必要です。");
        has_issues = true;
    }
    ...
    for (const auto& dev : serial_comms_) {
        ...
        if (!dev->isOpen()) {
            RCLCPP_WARN(this->get_logger(), "⚠️ シリアルデバイス %s の接続が失われています。", dev->getPort().c_str());
            has_issues = true;
        } else {
            // 接続が開いていても、実際に通信可能かチェック
            if (!dev->checkConnection()) {
                RCLCPP_WARN(this->get_logger(), "⚠️ シリアルデバイス %s は開いていますが、通信に問題があります。", dev->getPort().c_str());
                has_issues = true;
            }
        }
    }
    ...
    if (has_issues) {
        RCLCPP_WARN(this->get_logger(), "⚠️ シリアル接続に問題が検出されました。再接続を試行します。");
    }
